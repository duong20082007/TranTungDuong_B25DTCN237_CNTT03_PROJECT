...
int isTitleDuplicate(char title[], int currentId) {
    for (int i = 0; i < bookCount; i++) {
...
int isBorrowDateValid(Date borrowDate, Date returnDate) {
    if (borrowDate.year > returnDate.year) {
		return 0;
	}
    if (borrowDate.year == returnDate.year && borrowDate.month > returnDate.month) {
		return 0;
	}
    if (borrowDate.year == returnDate.year && borrowDate.month == returnDate.month && borrowDate.day > returnDate.day) {
		return 0;
	}
    return 1;
}
